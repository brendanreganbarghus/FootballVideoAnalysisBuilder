...
<body>
  ...
  <script>
    ...
    function renderRunControls() {
      ...
      processButton.disabled = running || !segment.processingSupported;
      ...
      if (segment.validationStatus === "passed") {
        ...
        segmentRunStatus.textContent = (
          segment.expectedFrames
            ? "Cold raw-video AI: " + segment.processedFrames + "/" +
              segment.expectedFrames + " sampled frames"
            : "Cold raw-video AI is processing locally"
        ) + (elapsed ? " · " + elapsed.toFixed(1) + "s elapsed" : "") +
          ". No prior detections, ball/player tracks, events, review labels, " +
          "or provider annotations are used. Events stay hidden.";
    ...
    function renderAiGate() {
      const ready = state.segment.state === "ready";
    ...
    prepareButton.addEventListener("click", async () => {
      ...
      if (![30, 60].includes(duration)) {
        segmentRunStatus.textContent =
          "Choose a 30- or 60-second review duration.";
        return;
      }
    ...
    processButton.addEventListener("click", async () => {
      ...
      segmentProgress.removeAttribute("value");
      segmentRunStatus.textContent =
        "Starting a cold raw-video run. Existing detections, tracks, events, " +
        "review labels, and provider annotations will not be used.";
      try {
